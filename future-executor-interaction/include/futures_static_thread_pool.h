#ifndef FUTURES_STATIC_THREAD_POOL_H
#define FUTURES_STATIC_THREAD_POOL_H

// This is based on executors/include/experimental/bits/static_thread_pool.h
// but with modifications dependent on the futures experiments
// Copied directly so that we can show it is usable with actual executors.
...
class futures_static_thread_pool;
...
class futures_static_thread_pool
{
  ...
  class executor_impl
  {
    friend class futures_static_thread_pool;
    futures_static_thread_pool* pool_;
    ProtoAllocator allocator_;

    executor_impl(futures_static_thread_pool* p, const ProtoAllocator& a) noexcept
  ...
  public:
    ...
    futures_static_thread_pool& query(execution::context_t) const noexcept { return *pool_; }
...
public:
  ...
  explicit futures_static_thread_pool(std::size_t threads)
  {
    for (std::size_t i = 0; i < threads; ++i)
      threads_.emplace_back([this]{ attach(); });
  }

  futures_static_thread_pool(const futures_static_thread_pool&) = delete;
  futures_static_thread_pool& operator=(const futures_static_thread_pool&) = delete;

  ~futures_static_thread_pool()
  {
    stop();
    wait();
  }
...
private:
  ...
  struct func : func_base
  {
    ...
    virtual void call()
    {
      ...
      fp.reset();
      futures_static_thread_pool::invoke(f);
    }
  ...
  struct thread_private_state
  {
    ...
    explicit thread_private_state(futures_static_thread_pool* p) : pool_(p) { instance() = this; }
...
#endif // FUTURES_STATIC_THREAD_POOL_H
